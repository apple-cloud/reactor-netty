...
public abstract class Transport<T extends Transport<T, C>, C extends TransportConfig> {
	 ...
	 *
	 * @param localAddressSupplier A supplier of the address to bind to.
	 *
	 * @return a new {@link Transport}
	 */
	public final T localAddress(Supplier<? extends SocketAddress> localAddressSupplier) {
		Objects.requireNonNull(localAddressSupplier, "localAddressSupplier");
		T dup = duplicate();
		dup.configuration().localAddress = localAddressSupplier;
		return dup;
	}
	...
	public final T metrics(boolean enable, Supplier<? extends ChannelMetricsRecorder> recorder) {
		if (enable) {
			T dup = duplicate();
			dup.configuration().metricsRecorder = recorder.get();
			return dup;
		}
		else if (configuration().metricsRecorder != null) {
			T dup = duplicate();
	 ...
	 * @return a new {@link Transport} reference
	 */
	public final T runOn(LoopResources loopResources, InternetProtocolFamily family) {
		Objects.requireNonNull(loopResources, "loopResources");
		Objects.requireNonNull(family, "family");
		T dup = duplicate();
		TransportConfig c = dup.configuration();
		c.loopResources = loopResources;
		c.preferNative = false;
		c.family = family;
		return dup;
	}
