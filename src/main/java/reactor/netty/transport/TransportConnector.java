...
public final class TransportConnector {
	...
	public static Mono<Channel> bind(TransportConfig config, ConnectionObserver connectionObserver) {
		Objects.requireNonNull(config, "config");
		Objects.requireNonNull(config.localAddress(), "localAddress");
		Objects.requireNonNull(connectionObserver, "channelInitializer");

		return initAndRegister(config, config.channelInitializer(config.channelOperationsProvider(), connectionObserver, null))
				.flatMap(channel -> {
					MonoChannelPromise promise = new MonoChannelPromise(channel);
					// "FutureReturnValueIgnored" this is deliberate
					channel.eventLoop().execute(() -> channel.bind(config.localAddress().get(), promise.unvoid()));
					return promise;
				});
	}
	...
	public static Mono<Channel> connect(TransportConfig config, SocketAddress remoteAddress,
			AddressResolverGroup<?> resolverGroup, ConnectionObserver connectionObserver) {
		Objects.requireNonNull(config, "config");
		Objects.requireNonNull(remoteAddress, "remoteAddress");
		Objects.requireNonNull(resolverGroup, "resolverGroup");
		Objects.requireNonNull(connectionObserver, "channelInitializer");

		return initAndRegister(config, config.channelInitializer(config.channelOperationsProvider(), connectionObserver, remoteAddress))
				.flatMap(channel -> doResolveAndConnect(channel, config, remoteAddress, resolverGroup));
	}

	@SuppressWarnings("FutureReturnValueIgnored")
	static void doConnect(SocketAddress remoteAddress, @Nullable Supplier<? extends SocketAddress> localAddress, ChannelPromise connectPromise) {
		...
		channel.eventLoop().execute(() -> {
			if (localAddress == null) {
				...
				SocketAddress local = Objects.requireNonNull(localAddress.get(), "localAddress");
				// "FutureReturnValueIgnored" this is deliberate
	...
	static Mono<Channel> doResolveAndConnect(Channel channel, TransportConfig config,
		...
		try {
			...
			Supplier<? extends SocketAddress> localAddress = config.localAddress();
